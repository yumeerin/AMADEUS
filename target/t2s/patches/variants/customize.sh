# Firmware
if [ -d "$WORK_DIR/vendor/firmware/SM-G996B" ]; then
    EVAL "rm -rf \"$WORK_DIR/vendor/firmware/SM-G996B\""
fi
EVAL "mkdir -p \"$WORK_DIR/vendor/firmware/SM-G996B\""
SET_METADATA "vendor" "firmware/SM-G996B" 0 2000 755 "u:object_r:vendor_firmware_file:s0"

while IFS= read -r f; do
    BLOB="$(basename "$f")"
    if [[ "$BLOB" == "libsn100u_fw.so" ]]; then
        BLOB="nfc/$BLOB"
        if [ -d "$WORK_DIR/vendor/firmware/SM-G996B/nfc" ]; then
            EVAL "rm -rf \"$WORK_DIR/vendor/firmware/SM-G996B/nfc\""
        fi
        EVAL "mkdir -p \"$WORK_DIR/vendor/firmware/SM-G996B/nfc\""
        SET_METADATA "vendor" "firmware/SM-G996B/nfc" 0 2000 755 "u:object_r:vendor_firmware_file:s0"
    fi
